...
nextNode = (buffer)->
    ...
    clean = cleanLine( buffer.current() )
    ...
    if tagged?
        ...
        value = clean
        while (buffer.current().indexOf('*/') == -1) and ((clean = cleanLine(buffer.current())) or true) and (not tagRe.test(clean) and not nameRe.test(clean))
            value = value + "\n" + clean
            buffer.next()
            break if not buffer.isOpen()
        return {tag: "raw", value: value}
