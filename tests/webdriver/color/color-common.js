function generateScript(renderMethod) {
  var testScript = function() {
    var box = Crafty.e('2D, $renderMethod')
...
module.exports = function(QUnit, browser, renderMethod) {
  ...
  QUnit.test("Color - " + renderMethod, function(assert) {
    return browser
      ...
      .signalBarrier('initial').waitBarrier('initial').assertResemble('color-initial')
      .signalBarrier('opaque').waitBarrier('opaque').assertResemble('color-opaque')
      .signalBarrier('transparent').waitBarrier('transparent').assertResemble('color-transparent');
  });
