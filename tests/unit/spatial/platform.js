(function() {
  ...
  test("Supportable", function(_) {
    var ground = Crafty.e("2D, Ground").attr({ x: 0, y: 10, w: 10, h: 10 }); // y: 10 to 20
    var ground2 = Crafty.e("2D, Ground").attr({ x: 0, y: 15, w: 10, h: 10 }); // y: 15 to 25
    ...
    _.strictEqual(
      ent.y,
      ground.y - ent.h,
      "ent y should have been snapped to ground"
    );
    _.strictEqual(ent.ground, ground, "entity should be on ground");
    ...
    ent.bind("CheckLanding", function(candidate) {
      if (candidate === ground) this.canLand = false;
    });
    ent.y = 7;
  ...
  test("Gravity", function(_) {
    var player = Crafty.e("2D, Gravity").attr({ x: 0, y: 100, w: 32, h: 10 });

    _.strictEqual(
      player.velocity().y,
      0,
      "velocity should be 0 before activating gravity"
    );
    _.strictEqual(
      player.acceleration().y,
      0,
      "acceleration should be 0 before activating gravity"
    );
    _.strictEqual(
      player._gravityConst,
      500,
      "gravityConst should match default value"
    );
    ...
    _.strictEqual(
      player._gravityConst,
      0.3 * 50 * 50,
      "gravityConst should match new value"
    );
    _.strictEqual(player._gravityActive, false, "gravity should not be active");
    ...
    _.strictEqual(
      player._groundComp,
      null,
      "no ground component set before activating gravity"
    );
    ...
    _.strictEqual(
      player._groundComp,
      "platform2",
      "new ground component set after activating gravity"
    );
    _.strictEqual(
      player.velocity().y,
      0,
      "velocity should be 0 before stepping a frame"
    );
    _.strictEqual(
      player.acceleration().y,
      player._gravityConst,
      "acceleration should match gravity constant after activating gravity"
    );
    ...
    _.strictEqual(
      player._groundComp,
      "platform",
      "ground component changed after reactivating gravity"
    );
    _.strictEqual(
      player.velocity().y,
      0,
    ...
    _.strictEqual(
      player.acceleration().y,
      player._gravityConst,
      "acceleration should match gravity constant after activating gravity twice"
    );
    ...
    _.strictEqual(
      player.acceleration().y,
      player._gravityConst,
      "acceleration should match gravity constant after changing gravity constant while gravity is active"
    );
    _.strictEqual(
      player._gravityActive,
      true,
      "gravity should still be active"
    );
    ...
    _.strictEqual(
      player.velocity().y,
      1000,
      "velocity should not have been reset after changing gravityConst"
    );
    ...
    _.strictEqual(
      player.acceleration().y,
      0,
      "acceleration should be zero after deactivating gravity"
    );
    _.strictEqual(
      player.velocity().y,
      0,
      "velocity should be zero after deactivating gravity"
    );
    ...
    _.strictEqual(
      player._groundComp,
      "platform",
      "ground component didn't change after reactivating"
    );
    _.strictEqual(
      player.acceleration().y,
      player._gravityConst,
    ...
    _.strictEqual(
      player.velocity().y,
      0,
      "velocity should be still be zero immediately after deactivating gravity"
    );
  });
  ...
  test("Supportable - Tunneling", function(_) {
     ...
     *****************************************/
    ground = Crafty.e("2D, Floor").attr({ x: 0, y: 100, w: 30, h: 1 }); // x-extent [0, 30] y-extent [100, 101]
    ...
    _.strictEqual(
      player.dy,
      5,
      "player should have moved for an expected amount"
    );
    _.strictEqual(
      player.y + player.h,
      5 + 10,
      "player should be at expected position"
    );
    _.ok(
      player.y + player.h < ground.y,
      "player should not have landed on ground"
    );
    player.y = 0;
    player.resetMotion();
    ...
    _.strictEqual(
      player.y + player.h,
      ground.y,
      "player should have landed on ground"
    );
    player.y = 0;
    player.resetMotion();
     ...
     *****************************************/
    ground = Crafty.e("2D, Floor").attr({ x: 100, y: 0, w: 1, h: 30 }); // x-extent [100, 101] y-extent [0, 30]
     ...
     *****************************************/
    ground = Crafty.e("2D, Floor").attr({ x: 99, y: 99, w: 2, h: 2 }); // x-extent [99, 101] y-extent [99, 101]
    ...
    _.ok(
      ground.x + ground.w < player.x,
      "player should not have landed on ground"
    );
    _.ok(
      ground.y + ground.h < player.y,
      "player should not have landed on ground"
    );
    player.x = 0;
    player.y = 0;
    ...
    _.strictEqual(
      landedCount,
      7,
      "player should have landed correct number of times"
    );
    _.strictEqual(
      liftedCount,
      7,
      "player should have lifted correct number of times"
    );
  });
  ...
  test("Integrationtest - Gravity", function(_) {
    ...
    var ground = Crafty.e("2D, platform").attr({ x: 0, y: 280, w: 600, h: 20 });
