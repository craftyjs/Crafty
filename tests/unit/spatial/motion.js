(function() {
  ...
  test("Motion", function(_) {
    ...
    var ent = Crafty.e("2D, Motion, AngularMotion").attr({ x: 0, y: 0 });
    ...
    ent.velocity().y -= 1;
    _.ok(
      ent.velocity().equals(new Vector2D(v0.x + 1, v0.y * 2 - 1)),
      "linear velocity should be <3,9>"
    );
    ent.arotation += 5;
    ...
    var dt = 0.02; // 20 ms in 1 frame is the default
    ...
    dt = 0.02; // 20 ms in one frame
    var dPos = new Vector2D(a).scale(0.5 * dt * dt).add(v0.clone().scale(dt)),
      dPos_r = v0_r * dt + 0.5 * a_r * dt * dt;

    _.ok(
      ent.motionDelta().equals(dPos),
      "delta should be equal to the new position"
    );
    ...
    _.strictEqual(
      ent.rotation,
      dPos_r,
      "entity rotation should match calculated"
    );
    var v1 = new Vector2D(a).scale(dt).add(v0),
      v1_r = a_r * dt + v0_r;
    ...
    _.strictEqual(
      ent.vrotation,
      v1_r,
      "angular velocity should match calculated"
    );
  ...
  test("Motion - CCDBR", function(_) {
    var ent = Crafty.e("2D, Motion").attr({ x: 0, y: 0, w: 10, h: 10 });
    var ccdbr, ccdbr2;
    ...
    _.strictEqual(
      ccdbr._x,
      0,
      "tunneling rectangle property matches expected value"
    );
    _.strictEqual(
      ccdbr._w,
      20,
  ...
  test("Motion - changing vector", function(_) {
    var ent = Crafty.e("2D, Motion").attr({ x: 0, y: 0 });
  ...
  test("Motion - NewDirection Event", function(_) {
    var e = Crafty.e("2D, Motion").attr({ x: 10, y: 10 });
    e.vx = 0;
    e.vy = 0;
    e.ay = 0;
    e.ax = -0.3;
  ...
  test("AngularMotion - NewRotationDirection Event", function(_) {
    var e = Crafty.e("2D, AngularMotion").attr({ x: 10, y: 10 });
    e.vrotation = 0;
    e.arotation = -0.3;
  ...
  test("Motion - Events", function(_) {
    var e = Crafty.e("2D, Motion, AngularMotion").attr({ x: 0, y: 0 });
    ...
    e.one("Rotated", function(oldValue) {
      _.strictEqual(
        oldValue,
        old_rotation,
        "[6] - Old rotation matches cached value"
      );
    });
    e.vrotation = -1;
    Crafty.timer.simulateFrames(1);
    ...
    e.one("Rotated", function(oldValue) {
      _.strictEqual(
        oldValue,
        old_rotation,
        "[7] - Old rotation matches cached value"
      );
    });
    e.vrotation = 1;
    Crafty.timer.simulateFrames(1);
    ...
    _.strictEqual(
      newRotationDirectionEvents,
      4,
      "NewRotationDirection fired 4 times."
    );
