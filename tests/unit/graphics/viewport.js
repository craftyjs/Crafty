(function() {
  ...
  test("rect", function(_) {
    ...
    _.strictEqual(
      rect1._x,
      -Crafty.viewport._x,
      "rect property matches expected value"
    );
    _.strictEqual(
      rect1._y,
      -Crafty.viewport._y,
  ...
  test("scroll using _x, _y", function(_) {
    ...
    _.strictEqual(
      before.x - Crafty.domHelper.translate(e.x, e.y).x,
      100,
    ...
    _.strictEqual(
      before.y - Crafty.domHelper.translate(e.x, e.y).y,
      70,
  ...
  test("Viewport resizing", function(_) {
    ...
    _.strictEqual(
      ...
      "Initial canvas size matches viewport"
    );
    _.strictEqual(
      Crafty.stage.elem.style.width,
      Crafty.viewport.width + "px",
      "Initial stage size matches viewport"
    );
    ...
    _.strictEqual(
      layer._canvas.width,
      Crafty.viewport.width,
      "Canvas size matches viewport after change"
    );
    _.strictEqual(
      Crafty.stage.elem.style.width,
      Crafty.viewport.width + "px",
      "Stage size matches viewport after change"
    );
    ...
    _.strictEqual(
      ...
      h + 10,
      "Viewport increased in width"
    );
    _.strictEqual(
      layer._canvas.height,
      Crafty.viewport.height,
      "Canvas size matches viewport after change"
    );
    _.strictEqual(
      Crafty.stage.elem.style.height,
      Crafty.viewport.height + "px",
      "Stage size matches viewport after change"
    );
  });
  ...
  test("follow", function(_) {
    ...
    _.strictEqual(
      Crafty.viewport._x,
      -(Crafty.viewport.width / 2 + 100),
      "Center viewport on entity.x"
    );
    _.strictEqual(
      Crafty.viewport._y,
      -(Crafty.viewport.height / 2 + 70),
      "Center viewport on entity.y"
    );
  });
  ...
  test("pan", function(_) {
    ...
    _.strictEqual(
      Crafty.viewport._x,
      -100,
      "Pan all the way when all the time is spent"
    );
  ...
  test("pan with easing", function(_) {
    ...
    _.strictEqual(
      Crafty.viewport._x,
      -25,
      "Pan quarter of the way on half the time"
    );
    Crafty.timer.simulateFrames(5);
    _.strictEqual(
      Crafty.viewport._x,
  ...
  test("zoom", function(_) {
    ...
    _.strictEqual(
      Crafty.viewport._x,
      Crafty.viewport.width / 4,
      "move all the way when all the time is spent"
    );
    _.strictEqual(
      Crafty.viewport._y,
      Crafty.viewport.height / 4,
      "move all the way when all the time is spent"
    );
  ...
  test("centerOn", function(_) {
    ...
    _.strictEqual(
      Crafty.viewport._x,
      -e.w / 2 + Crafty.viewport.width / 2,
      "Entity centered after exact duration"
    );
    _.strictEqual(done, 1, "CameraAnimationDone has fired once");
    done = 0;
    Crafty.one("CameraAnimationDone", panDone);
    ...
    _.strictEqual(
      Crafty.viewport._x,
      -e.w / 2 + Crafty.viewport.width / 2,
      "Entity still centered 10 frames later"
    );
    ...
    _.strictEqual(
      Crafty.viewport._x,
      -(e2.x + e2.w / 2 - Crafty.viewport.width / 2),
      "Entity centered from non-zero origin"
    );
    _.strictEqual(
      Crafty.viewport._y,
      -(e2.y + e2.h / 2 - Crafty.viewport.height / 2),
      "Entity centered from non-zero origin"
    );
  });
  ...
  test("pan and StopCamera", function(_) {
    ...
    _.strictEqual(
      Crafty.viewport._x,
      -50,
      "Pan still half way after camera has been stopped"
    );
  ...
  test("zoom and StopCamera", function(_) {
    ...
    _.strictEqual(
      Crafty.viewport._scale,
      2,
      "Zoom at half way after camera has been stopped"
    );
    _.strictEqual(done, 0, "CameraAnimationDone hasn't fired");
  });
  ...
  test("DOMtranslate", function(_) {
    ...
    clientX =
      Crafty.stage.x +
      Crafty.stage.elem.clientWidth -
      document.body.scrollLeft -
      document.documentElement.scrollLeft;
    clientY =
      Crafty.stage.y +
      Crafty.stage.elem.clientHeight -
      document.body.scrollTop -
      document.documentElement.scrollTop;
    craftyxy = Crafty.domHelper.translate(clientX, clientY);
    _.strictEqual(
      craftyxy.x,
      -Crafty.viewport._x + Crafty.viewport._width / Crafty.viewport._scale
    );
    _.strictEqual(
      craftyxy.y,
      -Crafty.viewport._y + Crafty.viewport._height / Crafty.viewport._scale
    );
  });
})();
